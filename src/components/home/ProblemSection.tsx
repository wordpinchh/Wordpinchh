  ...
  return (
    <section className="grid grid-cols-1 lg:grid-cols-2 border-b border-[rgba(255,255,255,0.07)]">
      ...
      <div className="px-5 sm:px-8 lg:px-16 py-10 lg:py-18 border-b lg:border-b-0 lg:border-r border-[rgba(255,255,255,0.07)]">
        ...
        <p className="mb-5" style={{ fontSize: 'clamp(15px,3.5vw,20px)', lineHeight: 1.8, color: 'var(--home-muted)', fontFamily: 'var(--font-inter)' }}>
          You've built something worth talking about. The problem? Content keeps falling to the bottom of the list.
        </p>
        <p className="mb-8" style={{ fontSize: 'clamp(15px,3.5vw,20px)', lineHeight: 1.8, color: 'var(--home-muted)', fontFamily: 'var(--font-inter)' }}>
          Sound familiar?
        </p>
        ...
        <ul className="mt-8 flex flex-col gap-0">
          {painPoints.map((point, index) => (
            <li
              key={index}
              className="flex items-start gap-3.5 py-4 border-b border-[rgba(255,255,255,0.07)] first:border-t first:border-[rgba(255,255,255,0.07)] transition-all duration-250 hover:bg-[rgba(255,255,255,0.02)]"
            >
              <div
                className="w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-px"
                style={{ background: 'var(--home-red-dim)', border: '1px solid rgba(204,32,32,0.2)' }}
              >
                <div className="w-2 h-2 rounded-full opacity-50" style={{ background: 'var(--home-red)' }} />
              </div>
              <span style={{ fontSize: 'clamp(13px,3vw,18px)', lineHeight: 1.6, color: 'var(--home-muted)', fontFamily: 'var(--font-inter)', fontStyle: 'italic' }}>
                {point}
              </span>
            </li>
          ))}
      ...
      {/* Right Column */}
      <div className="px-5 sm:px-8 lg:px-16 py-10 lg:py-18 flex flex-col justify-between gap-6">
        {/* Meanwhile Box */}
        <div
          className="rounded-2xl p-6 lg:p-8 relative overflow-hidden"
          style={{ background: 'var(--home-surface)', border: '1px solid var(--home-border-2)' }}
        >
          <div
            className="absolute -top-2.5 left-4 text-[120px] leading-none opacity-[0.03]"
            style={{ fontFamily: 'var(--font-inter)', color: 'var(--home-text)' }}
          >
            "
          </div>
          <div
            className="mb-3.5"
            style={{ fontFamily: 'var(--font-inter)', fontSize: 'clamp(11px,2.5vw,15px)', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--home-gold)' }}
          >
            Meanwhile
          </div>
          <p style={{ fontFamily: 'var(--font-inter)', fontSize: 'clamp(1.1rem,3.5vw,1.6rem)', lineHeight: 1.7, color: 'var(--home-text)', fontStyle: 'italic' }}>
        ...
        <div
          ...
          style={{ background: 'var(--home-lime-dim)', border: '1px solid rgba(202,255,74,0.15)' }}
        >
          <div
            className="flex items-center gap-2.5 mb-3"
            style={{ fontFamily: 'var(--font-fahkwang)', fontSize: 'clamp(11px,2.5vw,15px)', fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--home-lime)' }}
          >
            ↑ Search Has Changed
          </div>
          <p style={{ fontSize: 'clamp(15px,3.5vw,22px)', lineHeight: 1.7, color: 'rgba(202,255,74,0.65)', fontWeight: 300, fontFamily: 'var(--font-inter)', fontStyle: 'italic' }}>
